...
Fcompress::~Fcompress() { free_resources(this->root); }
...
void Fcompress::create_tree() {
  // creat min heap
  for (int i = 0; i < 128; i++) {
    if (code_table[i]->frequency) // execluding non occupied characters from
                                  // heap
...
void Fcompress::fill_table() { calculate_leaves(root, ""); }
...
void Fcompress::write_to_file() {
  ...
  while (!temp.empty()) { // Save table in header of compressed file
    c_node *node = temp.top();
    to_write += node->char_;
    code.assign(127 - node->huff_code.size(),
                '0'); // put 0's to fill 127 bit chunck
...
void Fcompress::create_tree_from_header() {
  ...
  unsigned char len;
  r_file.read(reinterpret_cast<char *>(&len),
              1); // reinterpret_cast<char*> casting char to char* in cpp;
  root = new c_node;
  for (int i = 0; i < len; i++) { // every node saved as 1byte of orignal char
                                  // and 16 byte of huff code
    ...
    while (
        code[0] !=
        '1') { // remove zerose added to 128 bit chunck till '1' we added before
      code = code.substr(1);
    }
...
void Fcompress::read_from_file() {
  ...
  r_file.seekg(
      -1, std::ios::end); // seek to last byte in file that represent zeroes;
  int last = r_file.tellg();
  r_file.read(&zeros, 1);
  r_file.seekg(1 + (17 * len),
               std::ios::beg); // seek to first byte of file encoded data
  ...
  while (!r_file.eof()) { // get the text byte by byte using unsigned char
